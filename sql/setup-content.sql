...
-- Adding index on type.
-- This is used throughout on the website

explain select * FROM `anax_content` where type = "page";
CREATE INDEX `type_name` ON `anax_content` (`type`);
...
SELECT `id`, `path`, `slug`, `type`, `title`, `created` FROM `anax_content`;
